...
# H_out and W_out height and width of the output matrix respectively
    ...
    #ENTER STUDENT CODE ABOVE ###
    macs= input_c * output_c * output_h * output_w * kernel_h * kernel_w  * input_n * output_n
...
def estimate_rom(tensors: List[MyTensor]):
    ...
    for tupi in tensors:
        #print(tupi.idx)
        #print(tupi)
        if tupi.is_const is True:
            lst=[]
            lst = tupi.shape
            if "32" in tupi.dtype:
                #print("int 32")
                for i in range(0,len(tupi.shape)):
                    rom_bytes_temp = rom_bytes_temp * lst[i]
                    #print(lst[i])
                    #print(rom_bytes_temp)
                rom_bytes_temp = rom_bytes_temp * 4
                #print("rom_bytes temp is ")
                #print(rom_bytes_temp)
                rom_bytes = rom_bytes + rom_bytes_temp
                rom_bytes_temp = 1
            ...
            if "8" in tupi.dtype:
                ...
                for i in range(0,len(tupi.shape)):
                    ...
                    #print(lst[i])
                    #print(rom_bytes_temp)
                rom_bytes_temp = rom_bytes_temp * 1
                #print("rom_bytes_temp con is ")
                #print(rom_bytes_temp)
                rom_bytes = rom_bytes + rom_bytes_temp
                rom_bytes_temp = 1
    ...
    for tupi in tensors:
        if tupi.is_const is False:
            ...
            if "32" in tupi.dtype:
                #print("int 32")
                for i in range(0,len(tupi.shape)):
                    ram_bytes_temp = ram_bytes_temp * lst[i]
                ram_bytes_temp = ram_bytes_temp * 4
                ...
                if (c>best_case_ram_bytes):
                    print("c")
                    print(c)
                    print(best_case_ram_bytes)

                    best_case_ram_bytes = c

                ram_bytes = ram_bytes + ram_bytes_temp
                ram_bytes_temp = 1
                j=j+1
            ...
            if "8" in tupi.dtype:
                for i in range(0,len(tupi.shape)):
                    ram_bytes_temp = ram_bytes_temp * lst[i]
                ram_bytes_temp = ram_bytes_temp * 1
        ...
        worst_case_ram_bytes = ram_bytes
