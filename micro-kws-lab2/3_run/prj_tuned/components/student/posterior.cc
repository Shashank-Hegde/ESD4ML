...
esp_err_t PosteriorHandler::Handle(uint8_t* new_posteriors, uint32_t time_ms,
  ...
  for (size_t i = 0; i < posterior_category_count_; i++) {
      sum[i] -= samples[i][location];
      samples[i][location] = new_posteriors[i];
      sum[i] += samples[i][location];
      if(num_samples < posterior_history_length_) {
        moving_avg[i] = sum[i]/num_samples;
      } else {
        moving_avg[i] = sum[i]/posterior_history_length_;
      }  
  }
  location = (location + 1) % posterior_history_length_;

  /* Determine top candidate */
  size_t top_canidate_index = 0;
  uint8_t top_canidate_value = 0;
  for (size_t i = 0; i < posterior_category_count_; i++) {
    if (moving_avg[i] > top_canidate_value) {
      top_canidate_index = i;
      top_canidate_value = moving_avg[i];
    }
  }
  *top_category_index = top_canidate_index;
  ...
  /* Handle trigger */
  *trigger = false;
  if (top_canidate_value > (posterior_trigger_threshold_/posterior_history_length_)) {
    if(trigger_time == 0) {
      trigger_time = time_ms;
      *trigger = true;
    }
  
    if((time_ms - trigger_time) > posterior_suppression_ms_) {
      trigger_time = time_ms;
      *trigger = true;
    }
  }
